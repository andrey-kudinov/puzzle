...
const puzzleStart = async id => {
  ...
  for (let i = 0; i < quiz.length; i++) {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      const answerHTML = `<label class="puzzle__input-container-js flex mb-5 cursor-pointer">
  ...
  document.querySelectorAll('.puzzle__input').forEach(item => {
    item.addEventListener('change', async e => {
      ...
      const inputsContainer = question.querySelectorAll(
        '.puzzle__input-container-js'
      )

      if (answer.textContent.trim() == quiz[question_id].correct_answer) {
        correctAnswersCounter++
        const correctAnswersCounterHTML = document.querySelector(
          '.puzzle__result-num'
        )
        correctAnswersCounterHTML.textContent = correctAnswersCounter

        const correctAnswersPercentHTML = document.querySelector(
          '.puzzle__progress-caption-js'
        )
        correctAnswersPercentHTML.textContent = `${Math.round(
          (correctAnswersCounter * 100) / 9
        )}%`

        const progressHTML = document.querySelector(
          '.puzzle__progress-bar-green-js'
        )
        progressHTML.style.width = `${Math.round(
          (correctAnswersCounter * 100) / 9
        )}%`
        ...
        const puzzleItem = document.querySelector(
          `.puzzle__item[data-id="${question_id}"]`
        )
        ...
        const puzzleItemsOpened = document.querySelectorAll(
          '.puzzle__item.opened'
        )
